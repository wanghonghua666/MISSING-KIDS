  ...
  try {
    ...
    const kindIndex =
      parts[0] === 'p' || parts[0] === 'reel' || parts[0] === 'tv'
        ? 0
        : parts[1] === 'p' || parts[1] === 'reel' || parts[1] === 'tv'
          ? 1
          : -1
...
function toInstagramPermalink(rawUrl: string): string | null {
  const post = instagramPostParts(rawUrl)
  return post ? `https://www.instagram.com/${post.kind}/${post.shortcode}/` : null
}

function toInstagramEmbedUrl(rawUrl: string): string {
  const post = instagramPostParts(rawUrl)
...
function toTikTokEmbedUrl(rawUrl: string): string | null {
  try {
    ...
    const id =
      videoIdx >= 0
        ? parts[videoIdx + 1]
        : parts[0] === 'embed' || parts[0] === 'embed-v2'
          ? parts[1]
          : null
...
export function resolveEmbed(rawUrl: string): ResolvedEmbed {
  ...
  if (host === 'soundcloud.com' || host === 'w.soundcloud.com' || host === 'on.soundcloud.com') {
